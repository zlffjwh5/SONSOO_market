      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <h1 class="mb-0 bread"><i class="fas fa-cloud"></i>펀딩 상품 수정<i class="fas fa-cloud"></i></h1>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-xl-8 ftco-animate">
			<form id="operForm" action="/funding/modifyFP" method="post" enctype="multipart/form-data" class="billing-form bg-light p-3 p-md-5">
							<h3 class="mb-4 billing-heading">상품 정보</h3>
							
	          	<div class="row align-items-end">
	          	<div class="col-md-6">
	                <div class="form-group">
	                	<label for="firstname">프로젝트 상태: 
						                  		<c:choose>
						    							<c:when test="${fp.fp_state==0}"> 펀딩 진행중</c:when>
						    							<c:when test="${fp.fp_state==1}"> 펀딩 마감(달성 성공)</c:when>
						    							<c:when test="${fp.fp_state==2}"> 펀딩 마감(달성 실패)</c:when>
						    					</c:choose>
		                 	 </label>
	  					  <input name="fp_state" id="fp_state" type="text" value="${fp.fp_state}" class="form-control" hidden>
	                </div>
	              </div>
	          	<div class="w-100"></div>
	          	<div class="col-md-6">
...
/* 	 $(document).ready(function(){
		...
		$("button[data-oper='delete']").on("click",function(e){
			...
			alert("X");
			 e.preventDefault();
			operForm.attr("action","/funding/deleteFP");
			operForm.submit();
		}); 
...
function deleteFP(){
 var id=$('#fp_id').val();
 location.href='/funding/deleteFP?fp_id='+id;
