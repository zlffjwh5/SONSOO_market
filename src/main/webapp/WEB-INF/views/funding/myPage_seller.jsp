...
<jsp:include page="../includes/menu.jsp" />
	...
	<section class="ftco-section bg-light">
    	<div class="container-fluid">
    		...
    		<form action="/funding/FProducts" method="get" >
    			<select name="category" class="txt" style="height: 34px; margin-bottom: 20px">
					<option value="%%">전체보기</option>
					<option value="도서">도서</option>  
					<option value="도서">게임</option>         
	            </select>
	    			...
	    			<div class="col-sm col-md-6 col-lg-3 ftco-animate">
	    				<div class="product">
	    					...
	    					<div class="text py-3 px-3">
	    						...
	    						<div class="d-flex">
	    							<div class="pricing">
			    						<p class="price" style="color: red"><span>
			    						<c:choose>
			    							<c:when test="${fpVO.fp_state==0}">펀딩 진행중</c:when>
			    							<c:when test="${fpVO.fp_state==1}">펀딩 마감(달성 성공)</c:when>
			    							<c:when test="${fpVO.fp_state==2}">펀딩 마감(달성 실패)</c:when>
			    						</c:choose>
			    						</span></p>
			    					</div>
		    					</div>
 ...
 <%--          <div class="col text-center">
    ...
    <!--  -->
<form id='actionForm' action="/funding/myProduct" method='get'>
</form> 
	<!-- Modal  추가 -->
		...
		$(document).ready(function(){
			...
			$(".move").on("click",function(e){
				...
				actionForm.append("<input type='hidden' name='fp_id' value='"+
						$(this).attr("href")+"'>");
				actionForm.attr("action","/funding/myProduct");	
				actionForm.submit();	
				});
		});
	</script>
