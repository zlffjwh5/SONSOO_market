...
<jsp:include page="../includes/menu.jsp" />
    	...
    	<div class="container">
    		<div class="row">
    			...
    			<div class="col-lg-6 product-details pl-md-5 ftco-animate">
					...
					결제는 <fmt:formatDate value="${fp.payDate}" pattern="yyyy-MM-dd"/>부터 7일 이내에 진행됩니다.
					</div>
						<div class="row mt-4">
          	...
          	<p><a href="#" class="btn btn-primary py-3 px-5" onclick="participate()">펀딩 참여하기</a>
          		<!-- 이름과 갯수 넘기기 -->
          		</p>
          		<br>
          	<hr>
	...
	<!-- 펀딩 참여하기 누를때 넘어가는 것 -->
<%-- 	<form id='orderForm'>
		<input type="hidden" name="fp_id" value='<c:out value="${fp.fp_id}"/>'>
...
function participate(){
	...
	var sellerId = '<c:out value="${fp.fp_seller}"/>';
	if(!memberId){
		alert("로그인 먼저 해주세요");
	...
	if(memberId==sellerId){
	      alert("자신이 주최한 펀딩은 참여할 수 없습니다.");
	      return;
	   }
	 var id=$('#fp_id').val();
	 var qty = $('#quantity').val();
	 location.href='/order/orderFundingView?fp_id='+id+'&qty='+qty;
}
...
$("button[data-oper='list']").on("click", function(e){
   ...
   operForm.attr("action","/funding/FProducts")
   operForm.submit();	       
}); 
