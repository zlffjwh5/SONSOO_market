...
function remove(){
	var con = confirm('공지사항을 삭제하시겠습니까?');
	if(con==true) {
		$("#modifyNotice").attr("action","remove");
		$("#modifyNotice").submit();
	}
}
      ...
      <div class="container">
        <div class="row block-12">
          <div class="col-md-8 order-md-last d-flex">
            ...
            <form id="modifyNotice" action="modifyNotice" method="post" class="p-5 contact-form">
              ...
              <div class="form-group">
                <label>제목</label><input type="text" name="title" id="title" value='<c:out value="${notice.title}"/>' class="form-control" required>
              </div>
              
              <div class="form-group">
                <label>요약 내용</label><input type="text" name="summary" id="summary" value='<c:out value="${notice.summary}"/>'  class="form-control" required>
              </div>
              
              <div class="form-group">
                <label>상세 설명</label><input type="text" name="content" id="content" value='<c:out value="${notice.content}"/>' class="form-control" required>
              </div>
              
              <div class="form-group">
              <label>썸네일 등록</label><input type="file" name="thumbnail" id="thumbnail" value='<c:out value="${notice.thumbnail}"/>' onchange="checkImage(this)" multiple="multiple" class="form-control">
              </div>
              ...
              <div class="form-group" style="margin-top: 30px;  text-align:right;">
  				...
  				<a href="/notice/list" class="btn btn-primary py-2 px-4">목록으로</a>
              </div>
